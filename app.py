...
st.markdown("""
    ...
    /* CUSTOMIZAÇÃO DA SIDEBAR (DARK NAVY) */
    ...
    /* Estilização do Menu de Navegação (Radio) */
    .stRadio > label { display: none; } 
    [data-testid="stSidebar"] .stRadio div[role="radiogroup"] { gap: 8px; padding-top: 15px; }
    
    /* Itens de Menu (Labels do Radio) */
    [data-testid="stSidebar"] .stRadio label { 
        color: #f1f5f9 !important; /* Cinza bem claro para leitura */
    ...
    /* Estilo para a opção selecionada (Streamlit usa um círculo interno, vamos realçar o texto) */
...
try:
    ...
    st.error("Erro: Falha na conexão com o servidor de dados.")
    st.stop()
...
# --- SIDEBAR (DESIGN CORPORATIVO REFINADO) ---
with st.sidebar:
    try: st.image("logo.png", use_container_width=True)
    except: pass
...
if menu == "01 DASHBOARD EXECUTIVO":
    ...
    st.markdown(f'<div class="insight-box"><h4>Unidade Analisada: {info.get("nome", "Não Identificada")}</h4><p>Sumário de indicadores de risco e criação de valor econômico.</p></div>', unsafe_allow_html=True)
...
# --- 3. PERFIL CORPORATIVO ---
    ...
    with st.form("f_emp"):
        ...
        colab = c2.number_input("Funcionários", value=int(safe_float(info.get('colab', 0))))
        ...
        if st.form_submit_button("Salvar Perfil"):
            save_data(st.session_state.group, "company_info", {"nome":nome, "setor":setor, "fundacao":fund, "colab":colab, "produto":prod, "desc":desc})
...
# --- 7. VIABILIDADE E VALOR ---
    ...
    with t2:
        g = st.slider("Taxa Crescimento Perpetuidade (g) %", 0.0, 10.0, safe_float(w_d.get('g_growth', 3.0)))
