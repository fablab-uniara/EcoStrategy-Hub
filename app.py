...
with st.sidebar:
    ...
    # Selic Global que afeta todo o sistema
    ...
    st.divider()
    st.caption("Foco Acadêmico e Profissional")

# --- 1. DASHBOARD ---
if menu == "Dashboard de Riscos":
    st.title("🚦 Intelligence Risk Dashboard")
    
    with st.expander("🎓 Como interpretar os Semáforos?"):
        st.write("Verde: Estável | Amarelo: Atenção | Vermelho: Risco Crítico.")
    ...
    hhi_val = 0
    try:
        hhi_val = sum([float(x)**2 for x in str(data.get('hhi', '0')).split(",") if x.strip()])
    except:
        hhi_val = 0

    w_obj = data.get('wacc', {})
    roi = safe_float(w_obj.get('roi'))
    w_final = safe_float(w_obj.get('wacc_final', 15.0))
...
# --- 3. DIÁRIO DE BORDO ---
    ...
    notas = st.text_area("Registros qualitativos da visita/entrevista", value=data.get('diary', ''), height=400)
...
# --- 4. MICRO ---
    ...
    t1, t2 = st.tabs(["Matriz de Porter", "HHI"])
    with t1:
        p = data.get('porter', {})
        col1, col2 = st.columns(2)
        p1 = col1.slider("Entrantes", 1, 5, int(p.get('p1', 3)))
        p2 = col1.slider("Fornecedores", 1, 5, int(p.get('p2', 3)))
        p3 = col1.slider("Clientes", 1, 5, int(p.get('p3', 3)))
        p4 = col2.slider("Substitutos", 1, 5, int(p.get('p4', 3)))
        p5 = col2.slider("Rivalidade", 1, 5, int(p.get('p5', 3)))
...
# --- 6. FINANCEIRO ---
    ...
    with c1:
        ...
        # Cálculo do WACC corrigido
        wacc_calc = (eq * (ke/100.0)) + ((1.0 - eq) * (kd/100.0) * 0.66)
        st.metric(label="WACC Final", value=f"{wacc_calc*100:.2f}%")
...
# --- 7. RELATÓRIO ---
    ...
    st.write(f"Empresa: {data.get('company_info')} | Selic: {selic_global}%")
    st.info(data.get('diary', 'Sem notas.'))
